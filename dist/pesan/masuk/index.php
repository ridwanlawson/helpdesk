...
<div class="main-content">
                      ...
                      <table class="table table-striped" id="table-1">
                        ...
                        <tbody>
                          <?php
                            ...
                            $query = "SELECT pesan.*, user.* FROM pesan, user WHERE pesan.id_penerima = $_SESSION[id] AND pesan.id_pengirim = user.id_user ORDER BY tanggal_pesan desc";
                            }else{
                            $query = "SELECT pesan.*, user.* FROM pesan, user WHERE pesan.id_penerima = $_SESSION[id] AND pesan.id_pengirim = user.id_user AND pesan.status_pesan = 'terkirim'  ORDER BY status_pesan desc,tanggal_pesan desc";
                            }
                            ...
                            <?php if ($_SESSION['level']=='user'): ?>
                              <td>
                                 <?php if ($data->status_pesan=='selesai teknisi'): ?>
                                    <a class="btn text-light btn-primary" href="<?php echo $_SESSION['direct'].'selesai.php?id='.$data->id_pesan.'&hd=pesan&fd=masuk'?>">Selesai</a>
                                 <?php endif ?>
                              </td>
                            <?php endif ?>
                          </tr>
  ...
  <?php
    $no = 1;
    $query = "SELECT pesan.*, user.* FROM pesan, user WHERE pesan.id_penerima = $_SESSION[id] AND pesan.id_pengirim = user.id_user AND pesan.status_pesan != 'selesai' ORDER BY status_pesan desc,tanggal_pesan desc";
    echo mysqli_error($conn);
    $result = $conn->query($query);
    while ($data = $result->fetch_object()) {
   ?>
